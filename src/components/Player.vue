<template>
	<div class="w-full h-screen flex flex-col bg-dark overflow-hidden" id="demo_wrap" style="height: 90vh">
		<div class="flex flex-row justify-center w-full m-auto relative z-10" id="upper-div" style="height: 90%">
			<div class="flex flex-col w-full md:w-5/12 bg-dark rounded-lg">
				<div class="m-auto w-4/5 mt-4 mb-0">
					<div v-for="(audio, indexo) in audios.slice(index, index + 1)" :key="indexo" class="mb-4">
						...
						<p class="text-sm text-white mt-1">{{ audio.artist }}</p>
					</div>
					<div id="img-div" class="m-auto relative" style="width: 300px; height: 300px">
						<img id="player-img"
				...
				<div class="flex w-4/5 m-auto justify-between items-center mt-8 mb-4" id="control-panel">
			...
			<div class="w-7/12 hidden md:block">
				<ul class="w-full overflow-auto m-auto mb-2 bg-dark pt-2" style="max-height: 100%" id="journal-scroll">
					<li
						@click="selectSound(indexo)"
						:style="indexo == index ? '' : ''"
						:class="indexo == index ? 'bg-lightest text-white' : 'text-white'"
						class="flex py-1 rounded cursor-pointer w-11/12 m-auto hover:bg-red-600"
			...
			<div class="flex w-11/12 h-16 items-center justify-around m-auto">
				...
				>
					...
					<div class="flex flex-col ml-2 font-semibold">
						<p class="text-white">{{ audio.title }}</p>
						<p class="text-xs text-white">{{ audio.artist }}</p>
					</div>
				</div>
				...
				<div class="w-1/5 flex md:w-2/12 m-auto items-center">
					<div class="w-3/12 md:w-2/12 hover:bg-dark rounded-full md:p-1 mr-2" @click="mute()">
						<svg
							v-if="mutePlayer"
					...
					<div class="w-9/12 md:w-10/12 m-auto relative">
						<div
							@click="volume($event)"
							ref="volBar"
							class="h-1 bg-red cursor-pointer rounded-full bg-gray-500 m-auto relative"
							style="width: 90%"
						>
							<div
								class="
						...
						>
							<span
								...
								class="w-3 h-3 md:w-4 md:h-4 bg-lightest absolute pin-r pin-b -mb-1 rounded-full shadow"
							></span>
						</div>
					</div>
				</div>
		...
		<div id="lower-list" class="w-12/12 md:block" style="overflow-y: auto; display: none; height: 35% !important">
			<ul class="w-full overflow-auto m-auto mb-2 bg-dark pt-2" style="" id="journal-scroll">
				<li
					@click="selectSound(indexo)"
					:style="indexo == index ? '' : ''"
					:class="indexo == index ? 'bg-lightest text-white' : 'text-white'"

  ...
  <div class="container-box no-select">
    ...
    >
      <div id="top_main" class="flex l_height">
        ...
        <div
          v-show="nav_show"
          class="w-56 bg-black h-full flex-none side-nav-div"
        >
          ...
          <div class="mx-2" style="color: white">
            ...
            >
              <i class="material-icons mr-3" style="max-width: 20px">{{
                page.icon
              }}</i>
              <p>
                {{ page.name }}
                {{ page.id === "home" ? "( " + all_songs + " )" : " " }}
              </p>
            </button>
          </div>
          <div class="mx-2" style="color: white">
            <h1 class="text-xs text-lightest tracking-widget uppercase">
              Playlist
            </h1>
            <button
              @click="favFun()"
          ...
          <div class="relative pt-0 mt-0" style="margin-top: -5px">
            ...
            <h1
              class="pl-2 mt-0 text-sm font-semibold text-white tracking wide"
            >
              Crafted with ❤️
            </h1>
        ...
        <div
          ...
          style="color: white; overflow-y: auto"
        >
          <!--header -->
          <div class="bg-dark w-full sticky top-0 p-2 h-16" style="z-index: 99">
            ...
            >
              <i
                class="material-icons mr-3 text-sm hover:text-white"
                style="zoom: 1.5"
                >menu</i
              >
            </button>
          ...
          >
            <h1
              id="redirect_head1"
              class="text-2xl font-semibold text-white tracking-wider"
            >
              Redirecting...
            </h1>
            <h1
            ...
            <div class="w-full flex flex-wrap flex_div recent-class">
              ...
              >
                ...
                >
                  ...
                  >
                    <i class="material-icons text-2xl pt-1" style="color: white"
                      >play_arrow</i
                    >
                  </div>
                  <div
          ...
          >
            <div class="px-3 py-1 flex item center justify-between">
              <h1 class="text-2xl font-semibold text-white tracking-wider">
                Made for You
              </h1>
              <h2
                id="see_all_custom"
            ...
            <h2
              class="ml-2 mb-1 text-xs text-lightest text-white tracking-wider"
            >
    ...
    <div
      v-show="!closePlayer"
      class="bg-dark player-div"
      style="top: 0; height: 50vh; width: 100vw; z-index: 100"
    >
      <Player
        style="z-index: 100"
...
export default {
  ...
  methods: {
    showAllCustom() {
      if (this.tempCustoms.length < this.customs.length) {
        ...
        if (this.tempCustoms.length >= 24)
          this.tempCustoms = this.tempCustoms.slice(0, 24);
      ...
      if (
        ...
        gsap.fromTo(
          ".custom-class",
          { opacity: 0.1, y: -200 },
          { opacity: 1, y: 0, duration: 1.5, ease: "expo" }
        );
      }
    },
    showAllRecent() {
      if (this.tempRecents.length < this.recents.length) {
        ...
        if (this.tempRecents.length >= 12)
          this.tempRecents = this.tempRecents.slice(0, 12);
        else this.tempRecents = this.recents;
      }

      if (
        document.getElementById("see_all_recent").textContent.toLowerCase() ===
          "hide" &&
        this.tempRecents.length <= 12
      ) {
    ...
    async playlist(id) {
      ...
      await axios
        ...
        .then((res) => {
          ...
          if (
            document.getElementById(this.currentPlaying.id + "hover-recent") !==
            null
          ) {
            document
          ...
          if (
            document.getElementById(this.currentPlaying.id + "hover") !== null
          ) {
            document
              .getElementById(this.currentPlaying.id + "hover")
          ...
          document.getElementById("recents_head").textContent =
    ...
    sideNavs(id) {
      ...
      if (id === "home") {
        ...
        setTimeout(() => {
          document.getElementById("redirect_head1").textContent =
            "You have been redirected :)";
          document.getElementById("redirect_head2").textContent =
            "Avoid song repetition while uploading :)";
          document.getElementById("redirect_head3").textContent =
    ...
    showplayer() {
      if (!this.player) {
        ...
        document.getElementById("arrow_btn_tog").textContent =
          "arrow_drop_down";
      } else {
        let div = document.getElementsByClassName("player-div")[0].style;
    ...
    async recentPlay(recent_id) {
      ...
      if (
        ...
        document.getElementById(this.currentPlaying.id) !== null
      ) {
        document.getElementById(
          this.currentPlaying.id
        ).innerHTML = `<i class="material-icons text-2xl pt-1" style="color: white;">play_arrow</i>`;
      ...
      if (this.currentPlaying.playing) {
        ...
        let index = arrList.findIndex(
          (item) => item.id === this.currentPlaying.id
        );
    ...
    setRecent() {
      ...
      let index_in_recent_tracks = JSON.parse(
        localStorage.getItem("recent_tracks")
      ).indexOf(this.currentPlaying.id);
      if (index_in_recent_tracks < 0) {
        let recent_tracks = [
          ...JSON.parse(localStorage.getItem("recent_tracks")),
        ];
        ...
        recent_tracks.unshift(
          JSON.parse(localStorage.getItem("recent_tracks"))[
            index_in_recent_tracks
          ]
        );
        localStorage.setItem("recent_tracks", JSON.stringify(recent_tracks));
      }
    ...
    setFav(e) {
      ...
      if (fav_tracks.indexOf(arr_id[0]) >= 0) {
        ...
        let index_fav_custom = this.customs.findIndex(
          (item) => item.id === arr_id[0]
        );
        ...
        if (arr_id[2] === "custom") {
          document
            .getElementById(arr_id[0] + "-fav-custom")
            .parentElement.classList.remove("bgWhite");
          document
            .getElementById(arr_id[0] + "-fav-custom")
          ...
          if (document.getElementById(arr_id[0] + "-fav-recent") !== null) {
            document
              .getElementById(arr_id[0] + "-fav-recent")
              .parentElement.classList.remove("bgWhite");
            document
              .getElementById(arr_id[0] + "-fav-recent")
              .classList.remove("colorRed");
        ...
        if (arr_id[2] === "custom") {
          document
            .getElementById(arr_id[0] + "-fav-custom")
            .parentElement.classList.add("bgWhite");
          document
            .getElementById(arr_id[0] + "-fav-custom")
  ...
  async mounted() {
    ...
    gsap.fromTo(
      ".recent-class",
      { opacity: 0.1, x: -200 },
      { opacity: 1, x: 0, duration: 1.5, ease: "expo" }
    );
    gsap.fromTo(
      ".custom-class",
      { opacity: 0.1, x: -200 },
